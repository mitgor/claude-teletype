...
- [ ] **CAP-01**: `PrinterProfile` exposes `bold_on`, `bold_off`, `italic_on`, `italic_off`, `underline_on`, `underline_off` byte fields (empty bytes = capability not supported)
- [ ] **CAP-02**: `PrinterProfile` exposes a `buffer_bytes` integer field (default chosen per profile family) used to chunk writes in instant mode
