...
- [x] **CLI-02**: User can run `claude-teletype print` with no path to launch the file picker, then exit after printing
...
- [x] **CAP-03**: Renderer applies a documented fallback chain: italic → underline → plain when italic codes are absent; bold → underline → plain when bold codes are absent
- [x] **CAP-04**: Built-in Epson ESC/P, IBM PPDS, and HP PCL profiles ship with verified bold and italic sequences encoded
- [x] **CAP-05**: Built-in Juki, OKI, and Citizen profiles ship with whatever bold sequences are documented for the family; absent capabilities are left as empty bytes (no incorrect codes)
